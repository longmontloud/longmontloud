...
TRUSTED_VENUES = ['bootstrap brewing', '300 suns brewing', 'wibby brewing', 'bricks on main', 'the dickens', 'abbott & wallace', 'johnsons station', 'the barn', 'duets bistro']
...
def parse_wibby_brewing():
    ...
    try:
        ...
        for item in soup.select('.event-item'):
            ...
            full_url = "https://www.wibbybrewing.com" + link_tag['href'] if link_tag['href'].startswith('/') else link_tag['href']
            date_str = item.select_one('.event-date').get_text(strip=True)
            event_date = datetime.strptime(f"{date_str}, {datetime.now().year}", "%B %d, %Y")
            events.append({"title": title, "venue": "Wibby Brewing", "url": full_url, "date": event_date, "time_str": "6:00 PM"})
    except: pass
    return events
